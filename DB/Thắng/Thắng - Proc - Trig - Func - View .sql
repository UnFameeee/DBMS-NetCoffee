...
(
	...
	StatusEmployee nvarchar(100),					--Trạng thái đi làm hoặc không
	Email nvarchar(100),
...
DROP FUNCTION SearchEmployeesWithName
CREATE FUNCTION SearchEmployeesWithName (@search_name nvarchar(100))
RETURNS TABLE AS
	RETURN SELECT * FROM dbo.EMPLOYEE
		   WHERE FullName LIKE '%' + @search_name + '%'

GO
SELECT * FROM dbo.SearchEmployees(N'Thành')								--chạy function
...
DECLARE @EmpID nvarchar(100), @Position nvarchar(100)
BEGIN
	--Lấy ra mã nhân viên vừa thêm/cập nhật vào
	SELECT @EmpID = inserted.EmpID
	FROM inserted
	--TÌm vị trí của nhân viên đó
	SELECT @Position = EMPLOYEE.WorkID
	FROM EMPLOYEE
	WHERE EMPLOYEE.ID = @EmpID
	--Nếu mà là Quản Lý (QL) thì rollback
	IF(@Position = 'QL')
	BEGIN
		PRINT ('Cant Insert or Update because this is Manager!!!')
		ROLLBACK
	END
END
--Test nhập
INSERT INTO WORK VALUES ('NV5', 1, 'NV5')	--LC
--Test update
